...
class TxtToImg:
    ...
    def run(
       ...
       # 获取传入图片的宽度和高度
        ...
        draw_table.text(
            xy=(text_x, text_y),
            text='今日已签到' if is_sign else "今日未签到",
            fill="#6d786f",
            font=title_font,
            spacing=lines_space
        )
...
def user_2_img(nickname="", sign_num=1, bg_path="", user_id=0, group_id=0, data=None, is_sign=False):
    # 生成的图片名称
    ...
    try:
        ...
        logger.error(f"个人信息查询失败,图片生成失败: {e}")
        return False, None
